...
export class ProfileManager {
  ...
  async initialize() {
    try {
      ...
      if (await fs.access(this.profilesFile).then(() => true).catch(() => false)) {
        const data = JSON.parse(await fs.readFile(this.profilesFile, 'utf-8'));
  ...
  getProfileByName(nameOrId) {
    // First try by ID
    if (this.profiles.has(nameOrId)) {
      return this.profiles.get(nameOrId);
    }
  ...
  // Alias for getAllProfiles for CLI compatibility
  getProfiles() {
