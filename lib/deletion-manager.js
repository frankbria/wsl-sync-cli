...
export class DeletionManager {
    ...
    for (const deletion of deletions) {
      try {
        ...
        if (dryRun) {
          ...
          if (!await fs.access(deletion.fullPath).then(() => true).catch(() => false)) {
          ...
          if (moveToRecycleBin && this.canMoveToRecycleBin()) {
            try {
              ...
              await fs.rm(deletion.fullPath, { recursive: true, force: true });
  ...
  async createBackup(deletion) {
    ...
    await copyRecursive(deletion.fullPath, backupPath);
  ...
  async restoreFile(relPath, targetPath = null) {
    ...
    // Copy file back
    await copyRecursive(backup.backupPath, restorePath);
          ...
          if (op.recoverable && op.success) {
            ...
            if (await fs.access(metadataPath).then(() => true).catch(() => false)) {
              ...
              this.deletedFiles.set(op.file, {
                backupPath,
                metadata,
                canRestore: await fs.access(backupPath).then(() => true).catch(() => false)
              });
            }
          }
        }
